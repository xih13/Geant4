...
void HistoManager::Book()
{
  ...
  // in HistoManager.hh
  G4AnalysisManager* analysisManager = G4AnalysisManager::Instance(); // 获得实例 analysisManager
  analysisManager->SetVerboseLevel(1); // verboseLevel，输出信息的等级
  analysisManager->SetNtupleMerging(true); // mergeNtuple
      
  // Create directories 
  analysisManager->SetHistoDirectoryName("histogram");
  analysisManager->SetNtupleDirectoryName("ntuple");

  // 打开文件；********
  // Open an output file
  //
  G4bool fileOpen = analysisManager->OpenFile("AnaEx01"); // fileName
  if (! fileOpen) {
  ...
  // CreateH1 meaning the Histogram is One-dimensional‘ ’
  // id = 0
  ...
  // add by xh.********
  // Create 3st ntuple (id = 2)
  // 有问题，输出 ntuple3 中没有数据
  analysisManager->CreateNtuple("Ntuple3", "Test");
  analysisManager->CreateNtupleDColumn("Eabs"); // column Id = 0
  analysisManager->CreateNtupleDColumn("Egap"); // column Id = 1
  analysisManager->CreateNtupleDColumn("Labs"); // column Id = 2
  analysisManager->CreateNtupleDColumn("Lgap"); // column Id = 3
  analysisManager->FinishNtuple();
  // add by xh.********
  // 创建文件；********
...
{                
  ...
  analysisManager->FillNtupleDColumn(2, 0, energyAbs);
  analysisManager->FillNtupleDColumn(2, 1, energyGap);
  analysisManager->FillNtupleDColumn(2, 2, trackLAbs);
  analysisManager->FillNtupleDColumn(2, 3, trackLGap);
